		...
		<div class="container">
			...
			<form action="freeModify.wow" method="post", enctype="multipart/form-data">
				<table class="table table-striped table-bordered">
					...
					<tr>
						<th>글번호</th>
						<td>글번호<input type="hidden" name="boNo" value="${freeBoard.boNo}"> 글번호는 바꿀수 없지
						</td>
					...
					<tr>
						<th>비밀번호</th>
						<td><input type="password" name="boPass" value="${freeBoard.boPass}" class="form-control input-sm" required="required" pattern="\w{4,}" title="알파벳과 숫자로 4글자 이상 입력"> <span class="text-danger"> <span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span> 글 등록시에 입력한 비밀번호를 입력하세요.
						</span></td>
					</tr>
					<tr>
						<th>분류</th>
						<td><select name="boCategory" class="form-control input-sm" required="required">
								...
								<c:forEach items="${codeList}" var="code">
									<option value="${code.commCd}" ${freeBoard.boCategory eq code.commCd ? "selected='selected'" : ""}>${code.commNm}</option>
								</c:forEach>
						</select></td>
					</tr>
					<tr>
